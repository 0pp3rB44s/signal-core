...
def test_production_snapshot_opens_paper_trade_and_persists_event(tmp_path):
    ...
    # `instrument` is sanitised at the producer (market_features.instrument_context),
    # so it must no longer be a raw ContractSpec...
    assert not isinstance(snapshot.context["instrument"], ContractSpec)
    assert snapshot.context["instrument"]["symbol"] == SYMBOL
    assert "raw" not in snapshot.context["instrument"], "unbounded exchange payload must be dropped"
    # ...but `live` is still a live dataclass, so the writer's sanitiser stays
    # load-bearing: without it json.dumps still cannot encode this payload.
