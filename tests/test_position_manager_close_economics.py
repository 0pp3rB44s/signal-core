...
def test_confirmed_close_reconciles_on_every_path(tmp_path, reason):
    ...
    out = h.reconcile_closed_lifecycle(pos(), {"status": "CLOSED"}, reason)
...
def test_long_and_short_both_reconcile(tmp_path):
    for side in ("long", "short"):
        ...
        assert h.reconcile_closed_lifecycle(
            pos(direction=side), {"status": "CLOSED"}, "dead_trade_timeout") == "RECONCILED"
...
def test_history_absent_leaves_it_provisional(tmp_path):
    ...
    assert h.reconcile_closed_lifecycle(pos(), {"status": "CLOSED"}, "dead_trade_timeout") == "PROVISIONAL"
...
def test_ambiguous_history_does_not_guess(tmp_path):
    ...
    assert h.reconcile_closed_lifecycle(p, {"status": "CLOSED"}, "dead_trade_timeout") == "PROVISIONAL"
    assert h.rows == []
...
def test_second_processing_writes_nothing(tmp_path):
    ...
    assert h.reconcile_closed_lifecycle(pos(), {"status": "CLOSED"}, "dead_trade_timeout") == "ALREADY"
    assert h.rows == []
